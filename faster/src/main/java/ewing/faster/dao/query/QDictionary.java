...
public class QDictionary extends com.querydsl.sql.RelationalPathBase<Dictionary> {
    ...
    private static final long serialVersionUID = -2116535970;
    ...
    public void addMetadata() {
        ...
        addMetadata(dictionaryId, ColumnMetadata.named("dictionary_id").withIndex(1).ofType(Types.DECIMAL).withSize(26).notNull());
        addMetadata(name, ColumnMetadata.named("name").withIndex(2).ofType(Types.VARCHAR).withSize(128).notNull());
        addMetadata(parentId, ColumnMetadata.named("parent_id").withIndex(5).ofType(Types.DECIMAL).withSize(26));
        addMetadata(rootId, ColumnMetadata.named("root_id").withIndex(6).ofType(Types.DECIMAL).withSize(26));
